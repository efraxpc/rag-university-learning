      ...
      <div className="border-t border-slate-200 bg-white px-4 py-3">
        <div className="mx-auto max-w-3xl">
          <div className="relative mb-2" ref={summaryRef}>
            ...
            {showSummaryMenu && (
              <div className="absolute bottom-full left-0 z-10 mb-2 w-96 overflow-hidden rounded-lg border border-violet-200 bg-gradient-to-b from-amber-50 via-white to-violet-50 shadow-xl">
                <div className="flex items-center justify-between border-b border-violet-100 bg-gradient-to-r from-amber-100 to-violet-100 px-4 py-2.5">
                ...
                {summaryDocs === null ? (
                  <p className="px-4 py-4 text-sm text-slate-400">Cargando…</p>
                  ...
                  <>
                    <label className="flex cursor-pointer items-center gap-2 border-b border-violet-100 px-4 py-2.5 text-sm font-semibold text-violet-800 transition hover:bg-violet-100/60">
                      <input
                        type="checkbox"
                    ...
                    <ul className="chat-scroll max-h-64 space-y-1.5 overflow-y-auto p-2.5">
                      {summaryDocs.map((d) => {
                            ...
                            <label
                              className={`flex cursor-pointer items-start gap-2.5 rounded-md border px-3 py-2 transition ${
                                selected
                                  ...
                                  : "border-slate-200 bg-white/70 hover:border-violet-300 hover:bg-violet-50"
                              }`}
                            >
                              <input
                                type="checkbox"
                              ...
                              <span className="min-w-0">
                                <span className="block whitespace-normal break-words text-sm font-medium leading-snug text-slate-700">
                                  {d.title ?? d.filename}
                                </span>
                                {d.title && (
                    ...
                    <div className="border-t border-violet-100 bg-white/70 p-2.5">
                      <button
                        onClick={summarizeSelection}
                        disabled={selectedIds.size === 0}
                        className="w-full rounded-md bg-gradient-to-r from-amber-400 to-violet-500 px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:shadow-md hover:brightness-105 disabled:opacity-40"
                      >
